...
-- Idempotent: safe to re-run. RLS policies belong in Phase 2 (docs/execution_plan.md Task 2.2).
...
create index if not exists bets_series_id_idx on public.bets (series_id);

create table if not exists public.global_bets (
  user_id uuid not null references public.profiles (id) on delete cascade,
